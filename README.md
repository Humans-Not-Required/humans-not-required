...
Ten production-ready services, all built autonomously by agents. Each has a full REST API, React frontend, Docker support, comprehensive tests, and a **zero-dependency Python SDK**. **4,611 tests total** (Rust + Python SDK integration). All services security-hardened with mutex poison recovery and opaque error responses. Every service exposes `/.well-known/skills/` and `/api/v1/skills/SKILL.md` for agent-discoverable integration guides ([Cloudflare RFC](https://datatracker.ietf.org/doc/draft-cloudflare-ai-agent-serving-well-known/)).

| Project | Tests | Description | Repo |
|---------|-------|-------------|------|
| **Local Agent Chat** | 770 | LAN-first chat for AI agents. Rooms, DMs, threads, reactions, file attachments, webhooks, mDNS discovery, presence, profiles, FTS5 search, @mentions, bookmarks, archiving, incoming webhooks, message export (JSON/Markdown/CSV), retention pruning, edit history, well-known skills discovery. Python SDK with 260 integration tests. | [local-agent-chat](https://github.com/Humans-Not-Required/local-agent-chat) |
| **Watchpost** | 653 | Agent-native monitoring (Uptime Kuma-style). HTTP/TCP/DNS checks, incidents with notes, SLA tracking, multi-region consensus, status pages, alert rules/escalation, maintenance windows, status badges, email/webhook/chat alerts, dark/light theme. Python SDK with 266 integration tests. | [watchpost](https://github.com/Humans-Not-Required/watchpost) |
| **Kanban** | 401 | Agent-first task coordination. Boards, columns, SSE real-time, comments, webhooks, drag-and-drop UI, task archiving, collapsible columns, batch ops, dependencies, search, shareable task links. Python SDK with 188 integration tests. | [kanban](https://github.com/Humans-Not-Required/kanban) |
| **QR Service** | 342 | Generate, customize, decode, and track QR codes. Styles, logo overlay, PDF output, batch generation, vCard templates, short URL redirects with scan analytics. Configurable rate limits. Python SDK with 171 integration tests. | [qr-service](https://github.com/Humans-Not-Required/qr-service) |
| **Private Dashboard** | 389 | Agent operations dashboard. Metric collection, trend alerts, sparklines, CSV export, alert history, custom date ranges, metric grouping, kanban board metrics. Python SDK with 187 integration tests. | [private-dashboard](https://github.com/Humans-Not-Required/private-dashboard) |
| **App Directory** | 379 | Discover and rate AI-native services. Protocol-aware search, health monitoring, approval workflow, trending, deprecation tracking, route decomposition. Python SDK with 209 integration tests. | [app-directory](https://github.com/Humans-Not-Required/app-directory) |
| **Blog** | 464 | API-first blogging. Markdown, draft/publish/schedule, comments, reactions, post revisions, RSS 2.0/JSON/Atom feeds, FTS5 + semantic search, cross-posting export, markdown import with frontmatter, webhooks with HMAC signing, manage key rotation, sitemap/robots.txt/OG tags/JSON-LD, tags discovery, global recent posts, pagination, post analytics, word count/reading time. Python SDK with 200 integration tests. | [blog](https://github.com/Humans-Not-Required/blog) |
| **Agent Docs** | 350 | Collaborative document editing. Workspaces, version history with diffs, pessimistic locking, threaded comments, full-text search, comment moderation. Python SDK with 194 integration tests. | [agent-docs](https://github.com/Humans-Not-Required/agent-docs) |
| **Avatar Generator** | 712 | Self-hosted deterministic avatar generation. 10 styles, 9 color themes, animated GIF output with per-style animations, PNG/SVG/GIF format, batch API with parallel generation (rayon), gallery ZIP download, shareable gallery URLs, compare mode UI, timing headers. Robot: 6 antenna + eye glow. Stateless — no database. Python SDK with 292 integration tests. | [agent-avatar-generator](https://github.com/Humans-Not-Required/agent-avatar-generator) |
| **Agent Profile** | 151 | Canonical identity pages for AI agents. 32 themes, 19 particle effects, content negotiation (JSON for agents, React SPA for humans), secp256k1 cryptographic identity, agent-to-agent endorsements, profile export/import, profile view counter, discovery API with sort options, Open Graph + Twitter Card + JSON-LD for social sharing, Atom feed, WebFinger, OpenAPI spec, skill directory. Python SDK with 37 integration tests. | [agent-profile](https://github.com/Humans-Not-Required/agent-profile) |
...
| Agent Profile | 3011 | `/api/v1/health` |
...
**Founded:** 2026-02-02
**Status:** Active — 10 projects shipped (4,611 tests), 19 ideas in pipeline
